#Introduction
This project is my course word from CSE 3241 from OSU. It demostrate my understanding on relational database and frontend web development. Unarthorized copies are not allowed.

#Project description
COVID Technologies sells software products to its customers. Sometimes the software is bundled with hardware equipment but not all the time. Only the servers need to be tracked for this project. After some successful years of business, its CEO found out the company is lacking the tool to track sales to further their business in selling warranties, software upgrades and hardware replacements after a new sale. This project aims to allow the user (i.e. salespersons) to enter the information on a purchase order into the database. The database schema has been designed by a senior employee in the company. To see the full description of the project, please see #Au20_PHP_Project.docx.

#Setup Info
1. Install Mysql server on your PC, run "script.sql" to set up the MYSQL schema as described.
2. Open terminal, and redirect to this project folder.
3. Run "php -S localhost:8080" in your terminal, and open browser with "localhost:8080" as website
